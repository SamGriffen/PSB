...
NRF24::NRF24(spi_inst_t *port, uint16_t csn, uint16_t ce)
{
    ...
    spi_init(this->port, 9600 ); // Initialise communication with NRF24 at 1MHz
...
void NRF24::config(){
    ...
    write_reg(RX_PW_P0, 0b00000010); // Set RX payloads to have 16 bits
...
void NRF24::send_message(uint8_t* msg){
...
void NRF24::receive_message(uint8_t* msg){
    uint8_t reg = 0b01100001; // R_RX_PAYLOAD command - Reads RX payload. Payload to be deleted from RX fifo when complete

    csn_low();
    spi_write_blocking(port, &reg, 1);
    spi_read_blocking(port, 0xff, msg, 2); // TODO: Expects to read from SPI into an array of uint8_t - For 16 bit packet make a way to read both and then construct them into a 16 bit integer.
    csn_high();
}